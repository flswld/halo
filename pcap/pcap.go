...
func OpenDevice(name string) (*Handle, error) {
	...
	return nil, fmt.Errorf("device not found: %v", name)
}
...
func (p *Handle) ReadPacketData() (data []byte, ci CaptureInfo, err error) {
	p.mu.Lock()
...
func (p *Handle) getNextBufPtrLocked(ci *CaptureInfo) error {
	if !p.isOpen() {
		return io.EOF
	}
...
func (p *Handle) ZeroCopyReadPacketData() (data []byte, ci CaptureInfo, err error) {
	p.mu.Lock()
	err = p.getNextBufPtrLocked(&ci)
	if err == nil {
...
func CompileBPFFilter(linkType LinkType, captureLength int, expr string) ([]BPFInstruction, error) {
...
//		doSomething()
//	}
func NewBPF(linkType LinkType, captureLength int, expr string) (*BPF, error) {
	h, err := pcapOpenDead(linkType, captureLength)
	if err != nil {
		return nil, err
	}
...
func (p *Handle) LinkType() LinkType {
	return p.pcapDatalink()
}

// SetLinkType calls pcap_set_datalink on the pcap handle.
func (p *Handle) SetLinkType(dlt LinkType) error {
...
func (p *Handle) Resolution() TimestampResolution {
	if p.nanoSecsFactor == 1 {
		return TimestampResolutionMicrosecond
	}
	return TimestampResolutionNanosecond
}
