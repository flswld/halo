...
func (b *BPF) pcapOfflineFilter(ci CaptureInfo, data []byte) bool {
	var hdr pcapPkthdr
...
func pcapOpenDead(linkType LinkType, captureLength int) (*Handle, error) {
	err := LoadWinPCAP()
	if err != nil {
		return nil, err
	}
...
func (p *Handle) pcapDatalink() LinkType {
	ret, _, _ := syscall.Syscall(pcapDatalinkPtr, 1, uintptr(p.cptr), 0, 0)
	return LinkType(ret)
}

func (p *Handle) pcapSetDatalink(dlt LinkType) error {
